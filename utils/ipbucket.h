...
#ifndef _FCA_IPBUCKET_H_
#define _FCA_IPBUCKET_H_

#include "list.h"
#include "string.h"

/* we define fca_item_t.length as uint32_t for saving memory,
...
static inline void ipbucket_init(fca_ipbucket_t *ipb)
{
	int i;
...
static inline void ipbucket_destory(fca_ipbucket_t *ipb)
{
	int i;
...
static inline void ipbucket_add(fca_ipbucket_t *ipb, struct list_head *node, size_t size)
{
...
static inline void ipbucket_update(fca_ipbucket_t *ipb, struct list_head *node, size_t size)
{
	list_del(node);
	ipbucket_add(ipb, node, size);
}

static inline struct list_head *ipbucket_get(fca_ipbucket_t *ipb, size_t size)
{
	struct list_head *p;
...
static inline struct list_head *ipbucket_biggest(fca_ipbucket_t *ipb)
{
	int index;
