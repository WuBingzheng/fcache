...
ngx_http_jstore_check_cacheable(ngx_http_request_t *r)
{
    ...
    if(cache_control && cache_control->elts) {
        hh = cache_control->elts;
        for(i = 0; i < cache_control->nelts; i++) {

            if(ngx_strncasecmp(hh[i]->value.data, "max-age=", 8) != 0
                    || hh[i]->value.data[8] == '0') {
                return 0;
            }
        }
...
static void ngx_list_dup(ngx_list_t *d, ngx_list_t *s)
{
    *d = *s;
    if(s->last == &s->part) {
        d->last = &d->part;
...
ngx_http_jstore_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if(content_length >= 0 && ctx->copied_length != content_length) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "content-length not match: %d %d",
                content_length, ctx->copied_length);
        goto fail;
    }
    ...
    /* request headers */
    ngx_list_dup(&sr->headers_in.headers, &r->upstream->headers_in.headers);
