...
#define FCA_CONF_DONE	(char *)FCA_DONE
#define FCA_CONF_OK	(char *)FCA_OK
#define FCA_CONF_AGAIN	(char *)FCA_AGAIN


static fca_conf_t conf_cycle;
static fca_server_t default_server;
static LIST_HEAD(reserved_servers);
static LIST_HEAD(reserved_devices);

typedef struct command_s fca_conf_command_t;
typedef const char *conf_parse_f(fca_conf_command_t *, void *, char *);
...
static const char *conf_set_path(fca_conf_command_t *cmd, void *data, char *arg)
{
	char *dest = ((char *)data) + cmd->offset;

	strncpy(dest, arg, PATH_LENGTH);
	return (dest[PATH_LENGTH - 1] != '\0') ? "path too long" : FCA_CONF_OK;
}

/* handler of INT type argument */
static const char *conf_set_int(fca_conf_command_t *cmd, void *data, char *arg)
{
	char *endp;
...
static const char *conf_set_size(fca_conf_command_t *cmd, void *data, char *arg)
{
	char *endp;
	int shift;
...
static const char *conf_set_flag(fca_conf_command_t *cmd, void *data, char *arg)
{
	fca_flag_t flag;
	...
	*((fca_flag_t *)(((char *)data) + cmd->offset)) = flag;
	return FCA_CONF_OK;
}

static const char *conf_new_device(fca_conf_command_t *cmd, void *data, char *arg)
{
	fca_device_t *device;
	struct list_head *p;
	...
	if (!list_empty(&reserved_devices)) {
		...
		list_del(p);
		device = list_entry(p, fca_device_t, dnode);
	} else {
		device = malloc(sizeof(fca_device_t));
		if (device == NULL) {
			return "NoMem";
		}
	}
	bzero(device, sizeof(fca_device_t));
...
static const char *conf_new_server(fca_conf_command_t *cmd, void *data, char *arg)
{
	fca_server_t *server;
	struct list_head *p;
	char *endp;
	...
	if (!list_empty(&reserved_servers)) {
		...
		list_del(p);
		server = list_entry(p, fca_server_t, snode);
	} else {
		server = malloc(sizeof(fca_server_t));
		if (server == NULL) {
			return "NoMem";
		}
	}
...
#define COMMAND_NUMBER (int)(sizeof(g_commands) / sizeof(fca_conf_command_t))
static fca_conf_command_t g_commands[] = {
	/* global */
	{	"error_log",
		conf_set_path,
		offsetof(fca_conf_t, error_log)
	},
	{	"quit_timeout",
		conf_set_int,
		offsetof(fca_conf_t, quit_timeout)
	},
	{	"device_badblock_percent",
		conf_set_int,
		offsetof(fca_conf_t, device_badblock_percent)
	},
	{	"device_check_270G",
		conf_set_flag,
		offsetof(fca_conf_t, device_check_270G)
	},
	{	"device",
		conf_new_device,
		0
	...
	{	"capacity",
		conf_set_size,
		offsetof(fca_server_t, capacity)
	},
	{	"access_log",
		conf_set_path,
		offsetof(fca_server_t, access_log)
	},
	{	"connections_limit",
		conf_set_int,
		offsetof(fca_server_t, connections_limit)
	},
	{	"rcvbuf",
		conf_set_size,
		offsetof(fca_server_t, rcvbuf)
	},
	{	"sndbuf",
		conf_set_size,
		offsetof(fca_server_t, sndbuf)
	},
	{	"recv_timeout",
		conf_set_int,
		offsetof(fca_server_t, recv_timeout)
	},
	{	"request_timeout",
		conf_set_int,
		offsetof(fca_server_t, request_timeout)
	},
	{	"send_timeout",
		conf_set_int,
		offsetof(fca_server_t, send_timeout)
	},
	{	"expire_default",
		conf_set_int,
		offsetof(fca_server_t, expire_default)
	},
	{	"expire_force",
		conf_set_int,
		offsetof(fca_server_t, expire_force)
	},
	{	"keepalive_timeout",
		conf_set_int,
		offsetof(fca_server_t, keepalive_timeout)
	},
	{	"item_max_size",
		conf_set_size,
		offsetof(fca_server_t, item_max_size)
	},
	{	"key_include_host",
		conf_set_flag,
		offsetof(fca_server_t, key_include_host)
	},
	{	"key_include_fca_key",
		conf_set_flag,
		offsetof(fca_server_t, key_include_fca_key)
	},
	{	"key_include_query",
		conf_set_flag,
		offsetof(fca_server_t, key_include_query)
	},
	{	"server_dump",
		conf_set_flag,
		offsetof(fca_server_t, server_dump)
	},
	{	"shutdown_if_not_store",
		conf_set_flag,
		offsetof(fca_server_t, shutdown_if_not_store)
	},
	{	"passby_enable",
		conf_set_flag,
		offsetof(fca_server_t, passby_enable)
	},
	{	"passby_begin_item_nr",
		conf_set_int,
		offsetof(fca_server_t, passby_begin_item_nr)
	},
	{	"passby_begin_consumed",
		conf_set_size,
		offsetof(fca_server_t, passby_begin_consumed)
	},
	{	"passby_limit_nr",
		conf_set_int,
		offsetof(fca_server_t, passby_limit_nr)
	},
	{	"passby_expire",
		conf_set_int,
		offsetof(fca_server_t, passby_expire)
	},
	{	"status_period",
		conf_set_int,
		offsetof(fca_server_t, status_period)
	},
};
...
static int conf_parse_file(const char *filename, int deep)
{
	...
	while (1) {
		...
		if (msg_rc == FCA_CONF_AGAIN) {
			continue;
		}
		if (msg_rc != FCA_CONF_OK) {
			break;
		}
		...
		if (strcmp(cmd, "include") == 0) {
			...
			if (conf_parse_file(included_file, deep + 1) != FCA_OK) {
		...
		if (i < server_context_begin) {
			...
			context = list_empty(&conf_cycle.servers)
				? &default_server
				: list_entry(conf_cycle.servers.prev, fca_server_t, snode);
		}
...
fca_conf_t *conf_parse(const char *filename)
{
	...
	default_server.key_include_fca_key = 0;
	...
	/* parse */
	if (conf_parse_file(filename, 0) == FCA_ERROR) {
		return NULL;
	}
	return &conf_cycle;
}
