...
typedef int http_parse_f(fca_request_t *, char *, ssize_t);
struct http_header_s {
	string_t	name;
	http_parse_f	*handler;
};

static int http_parse_put_content_length(fca_request_t *r, char *p, ssize_t len)
{
	char *endp;
...
static int http_parse_host(fca_request_t *r, char *p, ssize_t len)
{
	r->host.base = p;
	r->host.len = len;
	return FCA_OK;
...
static int http_parse_fca_key(fca_request_t *r, char *p, ssize_t len)
{
	r->fca_key.base = p;
	r->fca_key.len = len;
	return FCA_OK;
...
static int http_parse_put_cache_control(fca_request_t *r, char *p, ssize_t len)
{
	char *value, *endp;
...
static int http_parse_put_expires(fca_request_t *r, char *p, ssize_t len)
{
...
static int http_parse_get_range(fca_request_t *r, char *p, ssize_t len)
{
	ssize_t start, end;
	char *begin = p;
	char *q;
...
static int http_parse_connection(fca_request_t *r, char *p, ssize_t len)
{
...
#define GENERAL_HEADERS \
	...
	{STRING_INIT("FCA-Key:"),  http_parse_fca_key}, \
...
static void http_add_put_headers(fca_request_t *r, char *base, ssize_t len)
{
	string_t *p;
	...
	if (r->put_header_nr == 0) {
		...
		if (p->base + p->len == base) {
			...
			if (r->put_header_nr == FCA_PUT_HEADERS_MAX) {
				return;
			}
			p++;
			p->base = base;
...
int http_request_parse(fca_request_t *r)
{
	...
	*r->buf_pos = '\0';
	...
	while (1) {
		...
		if (is_store && (h->handler == NULL || rc == FCA_DECLINE)) {
...
{
#define FCA_HTTP_PAGE(code, desc) \
        ...
        static struct http_page_s {
                ...
                FCA_HTTP_PAGE(201, "Created"),
                FCA_HTTP_PAGE(204, "No Content"),
                FCA_HTTP_PAGE(400, "Bad Request"),
                FCA_HTTP_PAGE(404, "Not Found"),
                FCA_HTTP_PAGE(413, "Request Entity Too Large"),
                FCA_HTTP_PAGE(416, "Requested Range Not Satisfiable"),
                FCA_HTTP_PAGE(500, "Internal Server Error"),
        };
	int i;
